...
export async function publicRoutes(fastify) {
    ...
    `).get(slug);
        ...
        if (resolvedTheme === 'auto') {
            if (age < 15)
                resolvedTheme = 'candy';
            else if (wish.recipient_gender === 'female' && age <= 45)
                resolvedTheme = 'rose';
            else if (wish.recipient_gender === 'male' && age <= 45)
                resolvedTheme = 'electric';
            else if (age > 45)
                resolvedTheme = 'gold';
            else
    ...
    fastify.post('/w/:slug/open', async (req, reply) => {
        const { slug } = req.params;
        const wish = await db.prepare('SELECT id FROM wishes WHERE slug = ?').get(slug);
        if (!wish) {
