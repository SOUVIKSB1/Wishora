...
export async function authRoutes(fastify) {
    ...
    `).run(userId, email, passHash, displayName, avatarUrl, gender, dob);
        ...
        const created = db.prepare('SELECT id, email, display_name, avatar_url, gender, user_dob, plan, created_at FROM users WHERE id = ?').get(userId);
    ...
    fastify.post('/auth/login', async (req, reply) => {
        ...
        const email = body.email.trim().toLowerCase();
        const passHash = hashPassword(body.password);
        const user = db.prepare('SELECT id, email, display_name, avatar_url, gender, user_dob, plan, password_hash FROM users WHERE LOWER(email) = ?').get(email);
    ...
    fastify.post('/auth/google', async (req, reply) => {
        ...
        WHERE id = ?
      `).run(body.display_name || user.display_name, user.avatar_url || body.avatar_url || null, body.gender || user.gender, body.user_dob || user.user_dob, googleId || user.google_id, user.id);
    ...
    fastify.get('/auth/me', async (req, reply) => {
        ...
        const user = db.prepare('SELECT id, email, display_name, avatar_url, gender, user_dob, plan, created_at FROM users WHERE id = ?').get(userId);
        if (!user) {
