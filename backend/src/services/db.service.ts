...
export const db = {
  ...
  async transaction<T>(fn: (...args: any[]) => Promise<T> | T): Promise<T> {
    if (isPostgres && pgPool) {
  ...
  prepare(sql: string) {
    return {
      get: async (...params: any[]) => db.get(sql, params),
      all: async (...params: any[]) => db.all(sql, params),
      run: async (...params: any[]) => db.run(sql, params)
    };
  }
};
...
async function seedMusicTracksOnly() {
  const countRes = await db.get<{ count: string | number }>('SELECT COUNT(*) as count FROM music_tracks');
  ...
  if (count === 0) {
    ...
    for (const t of tracks) {
      await db.run(
        'INSERT INTO music_tracks (id, title, artist, duration, genre, mood_tags, storage_url, is_premium) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [t.id, t.title, t.artist, t.duration, t.genre, t.tags, t.url, t.premium]
      );
    }
  }
}
...
async function seedWishTemplates() {
  const countRes = await db.get<{ count: string | number }>('SELECT COUNT(*) as count FROM wish_templates');
  const count = Number(countRes?.count || 0);
  ...
  if (count === 0) {
    ...
    for (const tpl of templates) {
      await db.run(
        'INSERT INTO wish_templates (id, title, content, category, tone, language, is_premium) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [tpl.id, tpl.title, tpl.content, tpl.category, tpl.tone, tpl.language, tpl.is_premium]
      );
    }
  }
}
...
export async function createDefaultUserFolders(userId: string) {
  const existingRes = await db.get<{ count: string | number }>('SELECT COUNT(*) as count FROM folders WHERE user_id = ?', [userId]);
