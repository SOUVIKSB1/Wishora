...
export function parseContactsCsv(csvContent: string): { rows: CsvContactRow[]; total: number; validCount: number } {
  ...
  const rows: CsvContactRow[] = parsed.data.map((raw) => {
...
function normalizeGender(g: string): string {
  if (g.includes('fem') || g === 'f' || g === 'girl' || g === 'woman') return 'female';
  if (g.includes('mal') || g === 'm' || g === 'boy' || g === 'man') return 'male';
  if (g.includes('child') || g.includes('kid')) return 'child';
  if (g.includes('non') || g.includes('nb')) return 'nonbinary';
  return 'unspecified';
}
