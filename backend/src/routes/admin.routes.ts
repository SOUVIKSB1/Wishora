...
async function verifyAdmin(req: any, reply: any): Promise<boolean> {
...
export async function adminRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/templates', async () => {
    const templates = await db.prepare('SELECT * FROM wish_templates ORDER BY is_premium ASC, category ASC').all();
    return { templates };
  });
  ...
  fastify.get('/admin/stats', async (req, reply) => {
    ...
    const [
      ...
      db.prepare('SELECT COUNT(*) as count FROM users').get() as Promise<any>,
      db.prepare('SELECT COUNT(*) as count FROM wishes').get() as Promise<any>,
      db.prepare("SELECT COUNT(*) as count FROM wishes WHERE status = 'generated'").get() as Promise<any>,
      db.prepare('SELECT COUNT(*) as count FROM contacts').get() as Promise<any>,
      db.prepare('SELECT COUNT(*) as count FROM reactions').get() as Promise<any>,
      db.prepare('SELECT COUNT(*) as count FROM music_tracks').get() as Promise<any>,
      db.prepare('SELECT COUNT(*) as count FROM wish_templates').get() as Promise<any>,
      db.prepare('SELECT COUNT(*) as count FROM system_notifications').get() as Promise<any>,
      db.prepare("SELECT COUNT(*) as count FROM users WHERE plan IN ('pro', 'vip', 'executive')").get() as Promise<any>
    ]);

    const stats = {
      total_users: Number(usersRes?.count || 0),
      total_wishes: Number(wishesRes?.count || 0),
      generated_wishes: Number(genWishesRes?.count || 0),
      total_contacts: Number(contactsRes?.count || 0),
      total_reactions: Number(reactionsRes?.count || 0),
      total_tracks: Number(tracksRes?.count || 0),
      total_templates: Number(templatesRes?.count || 0),
      total_notifications: Number(notifsRes?.count || 0),
      active_vip_users: Number(vipRes?.count || 0)
    };
  ...
  fastify.put('/admin/users/:id/plan', async (req, reply) => {
    ...
    await db.prepare('UPDATE users SET plan = ? WHERE id = ?').run(plan, id);
    const updated = await db.prepare('SELECT id, email, display_name, role, plan FROM users WHERE id = ?').get(id);
  ...
  fastify.delete('/admin/users/:id', async (req, reply) => {
    ...
    await db.transaction(async () => {
      ...
      const userWishes = await db.prepare('SELECT id FROM wishes WHERE user_id = ?').all(id) as { id: string }[];
      for (const w of userWishes) {
        await db.prepare('DELETE FROM wish_photos WHERE wish_id = ?').run(w.id);
        await db.prepare('DELETE FROM wish_opens WHERE wish_id = ?').run(w.id);
        await db.prepare('DELETE FROM reactions WHERE wish_id = ?').run(w.id);
        await db.prepare('DELETE FROM wish_versions WHERE wish_id = ?').run(w.id);
      }

      await db.prepare('DELETE FROM wishes WHERE user_id = ?').run(id);
      await db.prepare('DELETE FROM contacts WHERE user_id = ?').run(id);
      await db.prepare('DELETE FROM folders WHERE user_id = ?').run(id);
      await db.prepare('DELETE FROM system_notifications WHERE user_id = ?').run(id);
  ...
  fastify.delete('/admin/music/:id', async (req, reply) => {
    ...
    const { id } = req.params as { id: string };
    await db.prepare('DELETE FROM music_tracks WHERE id = ?').run(id);
  ...
  fastify.get('/admin/templates', async (req, reply) => {
    ...
    const templates = await db.prepare('SELECT * FROM wish_templates ORDER BY created_at DESC').all();
    return { templates };
  });
  ...
  fastify.post('/admin/templates', async (req, reply) => {
    ...
    const created = await db.prepare('SELECT * FROM wish_templates WHERE id = ?').get(id);
  ...
  fastify.put('/admin/templates/:id', async (req, reply) => {
    ...
    const updated = await db.prepare('SELECT * FROM wish_templates WHERE id = ?').get(id);
  ...
  fastify.delete('/admin/templates/:id', async (req, reply) => {
    ...
    const { id } = req.params as { id: string };
    await db.prepare('DELETE FROM wish_templates WHERE id = ?').run(id);
    return { success: true, id };
  });
  ...
  fastify.post('/admin/notifications', async (req, reply) => {
    ...
    const created = await db.prepare('SELECT * FROM system_notifications WHERE id = ?').get(id);
    return {
      success: true,
  ...
  fastify.delete('/admin/notifications/:id', async (req, reply) => {
    ...
    const { id } = req.params as { id: string };
    await db.prepare('DELETE FROM system_notifications WHERE id = ?').run(id);
    return { success: true, id };
  });
}
